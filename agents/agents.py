...
"""
import autogen
from tools import search_google_news, search_google
from autogen import AssistantAgent, UserProxyAgent, config_list_from_json
from config import config_list_qwen, config_list_gpt4  #, llm_func_config
from config import llm_func_config, llm_func_config_a


norm_planner = autogen.AssistantAgent(
    name="norm_planner",
...
def ask_planner(message):
    """"
    ask planner to:
    1. get a plan for finishing a task;
    2. verify the execution result of the plan and potentially suggest new plan.",
    """
    planner_user.initiate_chat(norm_planner, message=message)
...
autism_teacher = autogen.AssistantAgent(
    name="autism_teacher",
    llm_config=llm_func_config_a,
    system_message="""你是一个超级ChatGPT提示工程师，在自闭症教育、治疗领域是专家。我会告诉你关于自闭症儿童的一些分类，以前每个类型下的患者的特点，需要你切到一个自闭症指导老师的身份，针对这个分类下的患者做陪聊工作，从而帮助患者成长、生活。你说话应该足够简洁，不要长篇大论，患者不喜欢太复杂的内容，一次接收太多的内容会很有难度。把你对患者说的话用【】括起来，把你的思考过程用()括起来。
...
user_proxy_terminate = autogen.UserProxyAgent(
    ...
    function_map={"ask_planner": ask_planner, "search_google_news": search_google_news, "search_google": search_google},
)
...
user_proxy_auto = autogen.UserProxyAgent(
    ...
    system_message="""当你得到一个链接时，你可以通过咨询assistant，从Google News上得到一些内容。如果任务得以解决，请回 TERMINATE。否则，回复 CONTINUE，或者给出不能解决任务的原因。""",
)
...
user_proxy_admin = autogen.UserProxyAgent(
    name="user_proxy_admin",
    human_input_mode="ALWAYS",  # ask human for input at each step
    max_consecutive_auto_reply=5,
    #is_termination_msg=lambda x: x.get("content", "") and x.get("content", "").rstrip().endswith("TERMINATE"),
